...
- Nuevo plugin **Publish**: exporta toda la bóveda a un sitio web estático (HTML/CSS/JS) autocontenido, con un diseño inspirado en Obsidian Publish/Quartz en vez de una lista plana de páginas — una página por nota, wiki-links convertidos en enlaces relativos reales (reutilizando el mismo resolutor de wiki-links que usa la vista previa, no una reimplementación aparte), listo para subir tal cual a GitHub Pages, Vercel o cualquier hosting estático. `Tools → Publish Vault as Static Site...` o desde la paleta de comandos.
  - **Navegación persistente**: barra lateral con el árbol de carpetas/notas completo (colapsable, estado recordado por visitante) y buscador de títulos, presente en todas las páginas — compartida por un único fichero (`assets/nav-data.js`) que cada página carga vía `<script src>`, no `fetch()`, para que funcione igual abriendo el sitio directamente como fichero que publicado en un servidor.
  - **Esquema de la página de nota**: migas de pan con la ruta de carpetas, título visible como encabezado real (antes solo aparecía en la pestaña del navegador), índice "On this page" generado de los propios encabezados de la nota, y sección "Linked mentions" al final listando qué otras notas exportadas enlazan a esta.
  - **Grafo**: zoom con la rueda (centrado en el cursor) y paneo arrastrando el fondo — antes no había forma de alejar/acercar ni mover la vista. Las etiquetas de nodo solo se dibujan al hacer zoom o al pasar el cursor por encima (antes se pintaban las miles de notas a la vez, ilegible). El cálculo de repulsión pasó de comparar cada nota con todas las demás (inviable con miles de notas) a un grid espacial que solo compara vecinos cercanos.
  - **Corregido**: `getAllNotes()` devuelve notas "stub" en una bóveda grande (solo título/preview, contenido aún no leído del disco) — se detectaban por error como adjuntos y se descartaban todas, dejando el sitio vacío. Ahora cada nota exportable fuerza la carga completa de su contenido antes de renderizar. Una nota individual con frontmatter YAML inválido ya no aborta la exportación entera — se aísla, se cuenta aparte y las demás siguen exportándose.
  - Las notas privadas se excluyen siempre — ni se escriben, ni se listan, ni son enlazables (un wikilink hacia una nota privada queda igual de "roto" que uno hacia una nota que no existe, sin filtrar su título en ningún sitio, tampoco en la barra lateral).
  - Limitación conocida, dicha explícitamente en vez de implementada a medias: no copia imágenes/adjuntos referenciados en las notas (un `<img>` embebido saldrá roto en el sitio publicado) ni ofrece búsqueda de texto completo (solo por título) ni páginas de aterrizaje por carpeta (la miga de pan es texto plano, no enlaces) — se deja para una iteración futura.
  - El diálogo "Elige qué publicar" ahora tiene un árbol de carpetas de verdad (carpetas y subcarpetas colapsables, con "Expand all"/"Collapse all", ordenado igual que el árbol lateral de Jylos — carpetas antes que notas, cada grupo por nombre) además del modo de lista plana que ya existía, con un buscador que filtra en vivo y un botón para alternar entre ambos modos. Corrige de paso un bug de fondo en `FolderService.getRootFolders()`, no específico de este diálogo: en una bóveda respaldada por sistema de ficheros devolvía **cero** carpetas raíz (el filtro comparaba con `null`, pero ese backend siempre resuelve el padre de una carpeta de primer nivel como la carpeta sintética de la raíz del vault, nunca `null`) — en una bóveda real de 922 carpetas, esto aplanaba tanto el árbol del diálogo de publicar como la estructura de carpetas del propio sitio exportado.
  - El grafo de cada nota (mini-grafo en su propia página) ahora es interactivo (zoom con rueda, arrastrar para mover, igual que el grafo completo) y muestra de verdad las notas relacionadas — antes, si una nota tenía alguna relación, el JSON embebido salía con una coma de menos entre el nodo propio y el primer vecino, lo cual es sintaxis inválida y hacía que el script fallara en silencio sin pintar ni siquiera el nodo de la propia nota.
  - Los diálogos de "plantilla para copiar" (Dataview, Mermaid) ahora usan un cuadro de texto de verdad, seleccionable y copiable con los atajos habituales o un botón "Copy" explícito — antes el contenido se mostraba como una etiqueta de solo lectura de JavaFX, que no admite selección de texto en absoluto.
  - Los plugins integrados (Publish, Dataview, MCP Server, Outline, Calendario, Notas diarias, Tiempo de lectura, Copia de seguridad, Plantillas, IA, Contador de palabras, Índice) siguen ahora el idioma configurado en Jylos en vez de mostrar siempre sus menús y diálogos en inglés — nuevo `PluginI18n`, con cada plugin cargando su propio `messages_es.properties` empaquetado en su jar (`scripts/build-plugins.sh` ahora copia esos ficheros de recursos, algo que antes solo hacía con las clases compiladas).

- Corrige que las listas de tareas (`- [ ] pendiente`, `- [x] hecha`) se mostraran como texto literal `[ ]`/`[x]` en vez de una casilla real, tanto en la vista previa como en cualquier exportación a HTML — `MarkdownProcessor` no cargaba la extensión de listas de tareas de commonmark.
...
- Classloader de plugins acotado a un API explícita, en vez de que cada plugin pueda alcanzar cualquier clase interna de Jylos por reflexión sin que nada lo impida:
  - Nueva `PluginApiSurface`: lista única de paquetes (`plugin`, `data.models`, `service`, `event`, `graph`) y clases sueltas (`util.MarkdownProcessor`, `util.WikiLinkResolver`, `util.AttachmentType`) que un plugin puede alcanzar del classloader de la app — el resto de `com.example.jylos.*` queda fuera.
  - Nuevo `PluginClassLoader` (sustituye al `URLClassLoader` plano que usaba `PluginLoader`): una petición de una clase `com.example.jylos.*` fuera de esa lista nunca llega al classloader de la app, aunque este pudiera resolverla — falla con un `ClassNotFoundException` claro, con el nombre exacto de la clase y un puntero a la documentación. Todo lo demás (JDK, JavaFX, dependencias de terceros) sigue exactamente igual que antes.
  - Verificado con los 12 plugins integrados reales, cargados a través del `PluginLoader` de verdad (no una compilación en plano): los 12 cargan e inicializan sin fallos. Sabotaje real hecho sobre el propio límite: desactivar el chequeo deja pasar una clase no listada (`UiDialogs`) sin problema — confirmado en rojo, restaurado.
  - Nuevo test-guardia (`PluginContractGuardTest`) que escanea el código fuente de cada plugin integrado buscando referencias a `com.example.jylos.*` fuera de `PluginApiSurface` — misma lista que usa el classloader real, así que no pueden desincronizarse. Complementa (no sustituye) al test de comportamiento real (`PluginClassLoaderBoundaryTest`), que compila y carga un plugin de verdad para probar que la carga falla de hecho, no solo que el código fuente "no debería" hacer algo.
  - `docs/PLUGINS.md` actualizado con el estado real del ciclo de vida y el alcance exacto (y los límites) de esta protección.
  - Se implementó y se retiró en la misma sesión un checksum SHA-256 por JAR instalado: chocaba con la propia capacidad de primera línea del sistema de plugins ("añadir/quitar un plugin soltando o borrando un JAR"), rechazando como "manipulado" un JAR recompilado y vuelto a soltar con el mismo nombre — indistinguible, en bytes, de una manipulación real, sin firma ni modelo de confianza que los diferenciara. Documentado en `docs/PLUGINS.md` como alternativa considerada y descartada, para no volver a proponerla sin este contexto.

- Nueva suite de contrato de comportamiento entre los dos backends de notas (`NoteDAOFlagsContractTest`, con subclases para SQLite y filesystem): el mismo cuerpo de test corre contra `NoteDAOSQLite` y `NoteDAOFileSystem` para favorito, pinned, papelera/restauración y privado, releyendo siempre del backend real (no de caché en memoria) tras cada mutación. Hasta ahora, cada backend reimplementaba estos cuatro atributos por su cuenta (columnas SQL vs claves de frontmatter YAML) sin ninguna red que detectara que uno de los dos dejara de escribir o de leer un atributo — ya ha pasado dos veces en el historial reciente (bucle infinito en `TagDAOFileSystem.deleteTag()`, caché sin conectar en `NoteDAOFileSystem`), cada vez en un solo backend. No se crea ninguna capa de código compartida entre los DAOs (las mecánicas son demasiado distintas: transacción SQL frente a mover fichero + caché + metadata sidecar + sufijo de colisión) — es intencionadamente solo una suite de test, complementaria a `NoteAttributeParityGuardTest` (que sigue igual, y solo comprueba que existen los nombres de columna/clave, no el comportamiento). Sabotaje real hecho en ambas direcciones: comentada la escritura de `is_private` en `NoteDAOSQLite` → falla solo la subclase SQLite; comentada en `FrontmatterHandler` (filesystem) → falla solo la subclase filesystem; restaurado ambas veces, confirmado verde.
...
- Corrige que `scripts/build-plugins.ps1` pudiera reventar el empaquetado Windows entero (portable, exe y msi) en vez de solo saltarse el plugin que fallara. Causa real: el manifest de los plugins multi-fichero (bundles, como Dataview) se escribía con `-Encoding UTF8`, que en PowerShell añade BOM al principio del fichero — `jar.exe` no sabe parsear un `MANIFEST.MF` que empiece con BOM y falla con `IOException: invalid header field name`. Ese fallo, al ir por `2>&1` con `$ErrorActionPreference = "Stop"` activo en todo el script, se convertía en un error que mataba el proceso completo en vez de quedarse contenido en ese plugin, pese a que quien llama a este script (`package-windows.ps1`) ya lo trata como "best effort". Arreglados ambos: el manifest ahora se escribe en ASCII (su contenido siempre lo es, sin BOM posible), y las cuatro llamadas a `javac`/`jar` que capturan su salida por `2>&1` restauran `$ErrorActionPreference` alrededor, para que un aviso o fallo de un plugin concreto no pueda tumbar el resto del empaquetado.

- Corrige que `scripts/build-plugins.ps1` nunca mostrara el error real de compilación de un plugin (bundle) fallido — solo "ERROR: Failed to compile bundle X", sin ningún detalle. Causa: `$ErrorActionPreference = "SilentlyContinue"` no solo evita que un fallo de `javac`/`jar` mate el script entero (el problema original que se quería arreglar), también descarta el contenido del error antes de que llegue a mostrarse. Cambiado a `"Continue"` (el comportamiento por defecto de PowerShell: no lanza, sí muestra) en los cuatro puntos añadidos para ese arreglo original.

- Instalador Windows (`scripts/package-windows.ps1`): instalación por-usuario en vez de por-máquina (sin UAC/admin, como VS Code/Discord), activada. El asistente con banner/fondo de marca (construido a partir de `resources/images/banner.png`) se intentó pero queda desactivado: con WiX local ya sano (se descartó antes un falso positivo — `.tools\wix314` corrupto, sin `wix.dll`, que daba `Error: Invalid or unsupported type: [msi]` sin relación con esto), el propio `candle.exe` de jpackage falla al compilar el proyecto WiX generado (exit 104) en cuanto se activa `--resource-dir scripts\wix-resources`; jpackage no muestra el error real de WiX por consola, así que la causa exacta no se conoce todavía. Detalle completo y comando exacto que falla en `docs/PACKAGING.md`.

- Corrige que el plugin MCP nunca se hubiera podido compilar en ningún release oficial (local o de CI, en ninguna plataforma). Causa real: sus 19 dependencias de terceros (Jetty, Jackson, el SDK de MCP...) vivían vendorizadas como JARs sueltos en `plugins-source/.../mcp/lib/`, pero la regla `*.jar` del `.gitignore` (pensada para el output de Maven) también las excluía a ellas — nunca se subieron a git, así que esa carpeta llegaba vacía a cualquier clon del repo, incluido el de CI. `build-plugins.ps1`/`.sh` ahora resuelven esas dependencias con Maven de verdad (`mvn -f pom.xml dependency:build-classpath`, mismo mecanismo que ya usa el resto del proyecto) desde un `pom.xml` propio del bundle, en vez de JARs vendorizados — sin binarios de terceros en el repo, con verificación de checksum de Maven incluida. Verificado de extremo a extremo: coordenadas Maven reales extraídas de cada JAR vendorizado (no adivinadas), resolución contra Maven Central y compilación real de las 6 fuentes de mcp, todo comprobado antes de este cambio. `test-plugins.sh` y la documentación (`docs/PLUGINS.md`, `docs/MCP.md`, `scripts/README.md`, y sus versiones en español) actualizados para reflejar el nuevo mecanismo.

## [2.5.4] - 2026-09-02

- El contador de notas de cada carpeta del árbol lateral ahora es recursivo: incluye las notas de todas sus subcarpetas, no solo las que están directamente dentro. Se calcula recorriendo el árbol de carpetas real de abajo a arriba (cada carpeta suma su propio recuento directo más el de todos sus descendientes), en vez de intentar deducirlo del formato del id de carpeta — así funciona igual en bóveda de ficheros (ids tipo ruta) que en SQLite (ids opacos). Se recalcula tanto al refrescar el recuento de notas como al reconstruirse la forma del árbol (crear/mover/borrar carpetas), para no quedarse desactualizado en ninguno de los dos casos.

- La cabecera del panel de notas ya no dice "Notas - NombreCarpeta", solo el nombre de la carpeta. Nuevo botón de tipo interruptor (como el resto de botones de estado persistente de la aplicación: mostrar etiquetas, grafo local, etc. — se queda activado hasta volver a pulsarlo, no hay que mantenerlo apretado) que, activado, muestra todas las notas de la carpeta actual y de todas sus subcarpetas de forma recursiva; el propio código de carga de notas por carpeta consulta este estado en cada carga, así que si se cambia de carpeta desde el árbol lateral con el interruptor activado, la vista recursiva sigue aplicándose a la carpeta nueva.

- Corrige que ese botón nuevo saliera más ancho que el resto de botones de icono del mismo panel. Fuera del contexto de la barra principal o del grafo (que ya tienen su propia regla más específica fijando el tamaño), este tipo de botón solo tenía fijada la altura en la regla compartida de unificación, no el ancho — su relleno propio (6px arriba/abajo, 10px a los lados) lo dejaba más ancho que alto. Fijado el mismo cuadrado de 28×28px que ya usa el resto de botones de icono, sin afectar a los que sí caen bajo esas reglas más específicas (comprobado que miden igual que antes).

- Corrige que el modo concentración, al activarse con el panel derecho desplegado, lo dejara abierto pero vacío en vez de ocultarlo como al resto de paneles. El panel derecho vive dentro de un `SplitPane` (editor | panel), y un `SplitPane` sigue reservando el hueco dividido de un elemento aunque se le ponga invisible/no gestionado — así se colapsan el resto de paneles, pero éste en concreto siempre se ha colapsado en el resto de la aplicación estrechando su ancho a 0, no ocultándolo. El modo concentración era el único sitio que usaba la técnica equivocada para este panel; ahora usa la misma que ya usa el resto del código para colapsarlo y expandirlo.

- Eliminada la fila "Todas las notas" del árbol de carpetas: con el interruptor de "mostrar también las notas de las subcarpetas" del panel de notas, seleccionar la carpeta raíz de la bóveda con ese interruptor activado hace exactamente lo mismo, así que la fila era redundante. El comando de la paleta "Ir a todas las notas" y el acceso directo de teclado se mantienen — antes buscaban esta fila en el árbol y la seleccionaban, ahora cargan las notas directamente, sin pasar por el árbol. Limpiadas también todas las comprobaciones del identificador especial de esa fila que quedaban repartidas por seis ficheros (menú contextual, arrastrar y soltar, validación de mover carpetas, etc.) — ya no hacían nada, ninguna carpeta puede volver a llevar ese identificador.

- Nueva opción en Preferencias para desactivar el número de notas junto a cada carpeta del árbol lateral. Activada por defecto — la interfaz se comporta igual que antes hasta que se desactiva a mano.

- Adelgazada la cabecera de la vista de nota (edición y previsualización). Dos causas reales, no cosas de diseño: los botones de atrás/adelante (`.nav-btn`) tenían su propio relleno (4px/6px) en vez del cuadrado de 28×28px que ya usa el resto de botones de icono de la app, así que medían 30×26px — fijados al mismo estándar. Y el campo del título medía 37px de alto pese a que su CSS decía un relleno de solo 2px: todo `TextField` de JavaFX lleva además la clase implícita `text-field`, y esa regla base (relleno de 8px, declarada más abajo en la hoja de estilos) ganaba el empate de especificidad — el mismo tipo de bug ya corregido una vez esta sesión para el buscador. Reafirmado el relleno del título después de esa regla; ahora mide 25px, menos que los botones, sin recortar el texto (comprobado con acentos y letras con trazo descendente).
...
- Nueva opción "ancho de línea legible" en Preferencias (editor y vista previa), estilo Obsidian: cuando está activada, el contenido de la nota se limita a una columna centrada de 700px en vez de ocupar todo el ancho del panel; el margen es un `max-width` con márgenes automáticos normal, así que se reduce y desaparece solo al estrechar el panel, sin lógica de redimensionado. Desactivada por defecto — la interfaz se comporta exactamente igual que antes hasta que se activa a mano. Se aplica tanto al editor (CodeMirror, vía su mecanismo existente de tema reconfigurable) como a la vista previa (la misma regla CSS en la hoja de estilos generada), y sobrevive al cambiar de nota sin volver a aplicarse a mano.

- El "tamaño de letra" de Preferencias solo afectaba a la barra de herramientas, panel lateral, menús y diálogos — nunca al texto del editor ni al de la vista previa, pese a llamarse igual. Nueva opción separada "tamaño de letra de editor y previsualización" (como en Obsidian, que también distingue interfaz de contenido: son tres motores de render distintos — JavaFX, CodeMirror y WebView — donde el mismo número de píxeles no se ve igual de grande en los tres). Reutiliza el mecanismo de zoom del editor que ya existía (Ctrl y rueda del ratón / atajo), que hasta ahora no se guardaba entre sesiones ni tocaba la vista previa para nada; ahora ambas cosas quedan resueltas: el zoom del editor persiste igual que ya persistía el de la interfaz, y se aplica también al cuerpo del texto de la vista previa (no a los bloques de código, que mantienen su tamaño fijo a propósito, igual que en la mayoría de editores).

- Corrige que el "tamaño de letra de la interfaz" de Preferencias solo afectara a una parte de la aplicación (algunos botones y paneles), dejando fuera la lista de notas, el árbol de carpetas y la mayoría de diálogos "complejos" como los de Git. El mecanismo aplica el tamaño como estilo heredable en la raíz de la ventana, pero casi 200 reglas repartidas en las dos hojas de estilo fijaban su propio `font-size` en píxeles absolutos — la herencia es lo más débil que hay en CSS, así que cualquier regla explícita las dejaba congeladas sin que el ajuste llegara nunca. Se han convertido todas a unidades `em` (relativas al tamaño heredado, el mecanismo estándar de JavaFX para esto, no una unidad inventada), manteniendo la proporción visual actual entre ellas — solo cambia que ahora escalan de verdad. Además, cada diálogo (Git, gestión de plugins, etiquetas, etc.) es su propia ventana con su propia raíz, así que necesitaba que se le reenviara el valor por su cuenta; antes solo se le enviaba al cambiar de tema, así que un diálogo abierto justo después de mover el zoom en caliente (Ctrl+rueda) seguía viéndose con el tamaño anterior. Verificado con una medición real de altura de texto renderizado, no solo revisando que la hoja de estilos compilara: una celda de la lista de notas pasa de 29px a 40px de alto y el título de un cambio en el diálogo de Git de 17px a 28px al subir el tamaño de 13 a 22.

- Corrige el desalineado del árbol de carpetas que a veces aparecía al arrancar o tras reconstruir el árbol (carpetas hermanas al mismo nivel con distinto sangrado). No era un bug de la aplicación sino de un mecanismo interno de JavaFX (RT-19656, documentado en el propio código fuente del JDK): el ancho de la flecha de desplegar se guarda en una caché compartida por todo el árbol, que arranca con un valor por defecto de 18px y solo se corrige la primera vez que se dibuja una carpeta con hijos — cualquier carpeta sin hijos dibujada antes de ese momento se queda con el sangrado equivocado para siempre. Cada celda del árbol usa ahora una flecha propia de ancho fijo exactamente igual a ese valor por defecto (18px, no un número cualquiera menor: se comprobó que dejaba un desajuste constante de 2px entre carpetas con y sin hijos), así la caché nunca necesita corregirse y todas las filas usan siempre el mismo desplazamiento. De paso, el contador de notas de cada carpeta `(N)` no tenía columna fija — su posición dependía de lo largo que fuera el nombre de la carpeta de cada fila; ahora un espaciador lo empuja siempre al mismo borde derecho. Verificado con dos tests que miden la posición real del texto renderizado tras el layout (no solo que compile): antes del arreglo, dos carpetas hermanas quedaban a 20.5px y 22.5px; con nombres de carpeta muy distintos de longitud, el contador caía en x=40.5px frente a x=214.5px. Ambos casos, sabotaje incluido, confirmados en rojo con el valor exacto y en verde tras el arreglo.
