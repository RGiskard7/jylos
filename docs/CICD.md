...
- passes the tag version to Maven through the standard `revision` property;
- runs tests;
- packages Windows and Linux builds;
- collects the plain JAR and uber-JAR generated by Maven;
...
The local packaging scripts (`scripts/package-*.sh`/`.ps1`) produce these same
standardized asset names alongside their raw jpackage output, so a locally
built installer is drop-in identical to a CI-released one.

The JAR asset names are intentionally stable, like the native installer names.
Tools such as JBang can use `releases/latest/download/jylos-uber.jar` for the
latest release. Users who need a fixed release can use the same asset name under
a specific tag URL, for example `releases/download/v2.4.5/jylos-uber.jar`.
...
Builds and publishes the VitePress website (landing page + docs) to GitHub Pages.

Runs on:

- `push` to `main`, limited to changes under `site/**` and the workflow itself;
- `workflow_dispatch`.
...
(`site/package.json`). Local development:
...
- Maven produces `jylos-2.4.1.jar` and `jylos-2.4.1-uber.jar`;
- the release publishes those files as stable assets: `jylos.jar` and `jylos-uber.jar`;
...
Native packaging scripts follow the same rule:

- in CI, `JYLOS_RELEASE_VERSION` comes from the pushed tag;
- locally, set `JYLOS_RELEASE_VERSION` when you want to reproduce a release
  package version without editing `pom.xml`.
